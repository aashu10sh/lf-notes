...
export default class CategoryController extends ApiClient {
  ...
  async addCategoryToNote(categoryId: number, noteId: number): Promise<Result<any, ApiError>> {
    try {
      const data = await fetch(
        this.getUrl(`category/${noteId}`),
        {
          method: "POST",
          headers: this.getHeaders(),
  ...
  async removeCategoryFromNote(categoryId: number, noteId: number): Promise<Result<any, ApiError>> {
    try {
      const data = await fetch(
        this.getUrl(`category/${noteId}`),
        {
